...
Instance::Instance(std::vector<std::set<int>>& st_v,std::map<std::set<int>,int>& set2weight, long long int num_of_elem, std::string format):
    fixed_sets_(0),
    fixed_elements_(0)
{
    this->st_v = st_v;
...
void Instance::read_balas1996(std::ifstream& file)
{
    ...
    ElementId number_of_elements;
    // file >> number_of_sets >> number_of_elements;
    number_of_sets = st_v.size();
    number_of_elements = num_of_elem;
    ...
    ElementId e;
    ElementId e_number;
    for(int s = 0; s < st_v.size(); s++){
        e_number = st_v[s].size();
        std::set<int> a_set = st_v[s];
        number_of_arcs_ += e_number;
        for(auto elem = a_set.begin();elem!=a_set.end(); elem++){
            e = *elem;
            e--;
            assert(e >= 0);
